...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (endTimestamp - startTimestamp) / (1000 * 3600 * 24) + 1;
}
...
function formatDate(date) {
  ...
  const utcHour = dateObject.getUTCHours() % 12 || 12;
  const utcMinute = dateObject.getUTCMinutes().toString().padStart(2, '0');
  const utcSecond = dateObject.getUTCSeconds().toString().padStart(2, '0');
  const utcPeriod = dateObject.getUTCHours() >= 12 ? 'PM' : 'AM';
  return `${utcMonth}/${utcDay}/${utcYear}, ${utcHour}:${utcMinute}:${utcSecond} ${utcPeriod}`;
}
